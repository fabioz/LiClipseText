...
@echo X:\liclipsetext\org.brainwy.liclipsetext.feature\build_liclipsetext.bat

@echo If needed to update version:
@echo x:
@echo cd x:\liclipsetext
@echo python update_version.py 1.0.0
...
%DRIVE%
cd %BUILD_DIR%
git clone %BASE_LOCAL_LICLIPSE_GIT%
@echo git clone git://github.com/fabioz/LiClipseText.git -- this could be used when building against the base git instead of a local git
cd LiClipseText
git reset --hard
git clean -f -d -x
git checkout -f
...
@echo
@echo BRANCH: the branch to be used to do the build (e.g.: master/development/etc) -- it's recommended that you create your own branch from a base branch in liclipsetext and use it
@echo
@echo BASE_LOCAL_LICLIPSE_GIT: The directory where you checked out LiClipseText from git (the folder containing the 'plugins, features, builders, .git')
@echo
@echo BUILD_DIR: The directory that should be created for the build (the contents of the BASE_LOCAL_LICLIPSE_GIT will be checked out to this directory for the build)
@echo
